...
export default function Dashboard2() {
  ...
  const buscarCiclosManuais = async () => {
    ...
    try {
      ...
      if (manualFiltroInicio) {
        qs.append("inicio", new Date(manualFiltroInicio).toISOString());
      }

      if (manualFiltroFim) {
        qs.append("fim", new Date(manualFiltroFim).toISOString());
      }
  ...
  const salvarCicloManual = async () => {
    ...
    if (!Number.isFinite(totalPessoasNum) || totalPessoasNum <= 0) {
      return alert("Total de pessoas deve ser maior que 0.");
    }

    if (!Number.isFinite(pesoTotalNum) || pesoTotalNum <= 0) {
      return alert("Peso total deve ser maior que 0.");
    }
  ...
  useEffect(() => {
    buscarRegistros();
  }, [filtroInicio, filtroFim, busca]);

  useEffect(() => {
    buscarCiclosManuais();
  }, [manualFiltroInicio, manualFiltroFim]);
          ...
          <div className="manuais-container">
            <div className="manuais-header">
              <div>
                ...
                <span className="manuais-sub">Lista de todos os ciclos manuais cadastrados</span>
              </div>

              <div className="manuais-filtros">
                <div className="filtro-manuais-item">
                  <label>Início</label>
                  <input
                    type="datetime-local"
                    value={manualFiltroInicio}
                    onChange={(e) => setManualFiltroInicio(e.target.value)}
                  />
                </div>
                ...
                <div className="filtro-manuais-item">
                  ...
                  <input
                    type="datetime-local"
                    value={manualFiltroFim}
                    onChange={(e) => setManualFiltroFim(e.target.value)}
                  />
                </div>

                <button className="btn-refeicoes-primary btn-refeicoes-buscar" onClick={buscarCiclosManuais}>
                  Buscar
                </button>

                <button
                  className="btn-refeicoes-secondary"
                  onClick={() => {
                    setManualFiltroInicio("");
                    setManualFiltroFim("");
                  }}
                >
                  Limpar
            ...
            {loadingManuais ? (
              ...
              <div className="manuais-vazio">Nenhum ciclo manual encontrado.</div>
            ) : (
                    ...
                    <div className="manual-info">
                      <div className="manual-top">
                        ...
                        <span className="manual-med">
                          médio{" "}
                          {(c.totalPessoas > 0
                            ? Number(c.pesoTotal) / Number(c.totalPessoas)
                            : 0
                          ).toFixed(3)} kg
                        </span>
                      </div>
                    ...
                    registros.map((r) => (
                      <div className="registro-card" key={r.id}>
                        ...
                        <div className="registro-grid">
                          ...
                          {/* <div className="registro-item">
                            <span className="registro-label">Peso pessoa</span>
                            <span className="registro-value">{Number(r.pesoPessoa).toFixed(3)} kg</span>
                          </div> */}
