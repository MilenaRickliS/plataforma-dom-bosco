...
export default function Usuarios() {
  ...
  async function handleSalvar(e) {
    ...
    try {
      ...
      if (editando) {
        await axios.put(`${API}/api/usuarios`, fd, { params: { emailOriginal: editando.email } });
  ...
  function handleEditar(u) {
    setEditando(u);
    setForm({ nome: u.nome, email: u.email, senha: "", role: u.role, foto: null });
    setPreview(u.foto);
  }
  ...
  return (
    <div className="usuarios-container">
      <div className="inicio-menug">
        <Link to="/gestao-portais" className="voltar-adm"><IoIosArrowBack /> Voltar</Link>
      ...
      <div className="form-usuarios">
        ...
        <form onSubmit={handleSalvar}>
          ...
          <input type="email" placeholder="E-mail" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} disabled={!!editando} />
          <input type="password" placeholder={editando ? "Nova Senha (opcional)" : "Senha"} value={form.senha} onChange={(e) => setForm({ ...form, senha: e.target.value })} />
          <select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} className="select-usuarios">
          ...
          <label htmlFor="file-upload" className="upload-label-usuarios">
            <span className="upload-text">
              <FiUpload size={20} /> {form.foto ? form.foto.name : "Escolher foto"}
            </span>
          </label>
          ...
          {editando && (
            <button type="button" className="btn-cancelar" onClick={() => { setEditando(null); setForm({ nome: "", email: "", senha: "", role: "aluno", foto: null }); setPreview(null); }}>
              Cancelar
            </button>
          )}
        </form>
      ...
      <div className="usuarios-lista">
        ...
        <table>
          ...
          <tbody>
            {usuarios.map((u) => (
              <tr key={u.email}>
                <td><img src={u.foto || "/src/assets/user-placeholder.png"} className="foto-tabela" /></td>
                <td>{u.nome}</td>
                <td>{u.email}</td>
                <td>{u.role}</td>
                <td>
                  <button onClick={() => handleEditar(u)} className="btn-editar-usuario"><MdEdit /> Editar</button>
                  <button onClick={() => handleExcluir(u.email)} className="btn-excluir-usuario"><FaRegTrashAlt /> Excluir</button>
                </td>
              </tr>
            ))}
          </tbody>
